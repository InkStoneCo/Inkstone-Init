 ...
 * daemon 指令 - Daemon 管理
 */
export async function daemonCommand(action) {
    try {
        const codemindPath = ensureCodemindExists();
...
export async function mapCommand() {
    try {
        ...
        for (const file of sortedFiles) {
            ...
            // 按 backlink 數量排序
            const sorted = [...fileNotes].sort((a, b) => (b.properties.backlink_count || 0) -
            ...
            for (const note of sorted) {
                ...
                console.log(`  [[${note.properties.id}]] ${truncatedSummary} ${output.reference(count)}`);
                // 子筆記
                for (const child of note.children) {
                    ...
                    console.log(`    [[${child.properties.id}]] ${childTruncated} ${output.reference(childCount)}`);
                }
            }
            output.blank();
        }
...
export async function showCommand(id) {
    try {
        ...
        if (note.properties.file)
            output.item(`File: ${note.properties.file}`);
        if (note.properties.line)
            output.item(`Line: ${note.properties.line}`);
        ...
        if (note.children.length > 0) {
            ...
            for (const child of note.children) {
                ...
                output.note(child.properties.id, child.displayPath, truncate(summary, 50));
            }
            output.blank();
        }
        // Related
        ...
        if (related.length > 0) {
            ...
            for (const r of related) {
                ...
                console.log(`  ${dir} [[${r.note.properties.id}]] ${truncate(summary, 40)}`);
            }
        }
    }
    catch (error) {
 ...
 * tree 指令 - 樹狀顯示筆記
 */
export async function treeCommand(options) {
    try {
        const store = getStore();
        let notes;
        if (options.file) {
        ...
        output.blank();
        const printTree = (note, indent = 0) => {
            const prefix = '  '.repeat(indent);
            const summary = note.content[0]?.content || '';
            const count = note.properties.backlink_count || 0;
            console.log(`${prefix}├─ [[${note.properties.id}]] ${truncate(summary, 40)} ${output.reference(count)}`);
            for (const child of note.children) {
 ...
 * add 指令 - 新增筆記
 */
export async function addCommand(file, content, options) {
    try {
        const store = getStore();
        const note = store.addNote(file, content, options.parent);
 ...
 * edit 指令 - 編輯筆記
 */
export async function editCommand(id, content) {
    try {
        const store = getStore();
        const note = store.updateNote(id, content);
 ...
 * link 指令 - 建立連結
 */
export async function linkCommand(fromId, toId) {
    try {
        const store = getStore();
        const fromNote = store.getNote(fromId);
        const toNote = store.getNote(toId);
        if (!fromNote) {
        ...
        store.updateNote(fromId, newContent);
 ...
 * unlink 指令 - 移除連結
 */
export async function unlinkCommand(fromId, toId) {
    try {
        const store = getStore();
        const fromNote = store.getNote(fromId);
        if (!fromNote) {
...
export async function searchCommand(query, options) {
    try {
        ...
        for (const result of results) {
            ...
            console.log(`  [[${result.note.properties.id}]] ${result.note.displayPath} ${output.reference(count)}`);
            // 顯示匹配的內容
 ...
 * refs 指令 - 顯示引用
 */
export async function refsCommand(id) {
    try {
        const store = getStore();
        const note = store.getNote(id);
        if (!note) {
        ...
        const backlinks = store.getBacklinks(id);
        const related = store.getRelated(id, 1);
        ...
        if (outgoing.length === 0) {
            ...
            for (const r of outgoing) {
                const summary = r.note.content[0]?.content || '';
                output.note(r.note.properties.id, r.note.displayPath, truncate(summary, 50));
            }
        }
    }
    catch (error) {
 ...
 * orphans 指令 - 列出孤立筆記
 */
export async function orphansCommand() {
    try {
        const store = getStore();
 ...
 */
export async function popularCommand(options) {
    try {
        const store = getStore();
        ...
        for (let i = 0; i < popular.length; i++) {
            ...
            const count = note.properties.backlink_count || 0;
            const summary = note.content[0]?.content || '';
            console.log(`  ${i + 1}. [[${note.properties.id}]] ${truncate(summary, 40)} ${output.reference(count)}`);
        }
        output.blank();
        output.divider();
 ...
 * expand 指令 - 展開檔案中的筆記
 */
export async function expandCommand(file) {
    try {
        const store = getStore();
        ...
        const formatNote = (note, indent = 0) => {
            const prefix = '  '.repeat(indent);
            ...
            const backlinks = store.getBacklinks(note.properties.id);
            if (backlinks.length > 0) {
                console.log(`${prefix}  backlinks:: ${backlinks.map(bl => `[[${bl.properties.id}]]`).join(', ')}`);
                console.log(`${prefix}  backlink_count:: ${backlinks.length}`);
            }
            // 內容
            for (const line of note.content) {
                console.log(`${prefix}  ${'  '.repeat(line.indent)}- ${line.content}`);
            }
            // 子筆記
            for (const child of note.children) {
...
export async function scanCommand() {
    try {
        ...
        output.success(`Scan complete. ${result.filesScanned} files scanned, ${result.notesCreated} created, ${result.notesUpdated} updated.`);
...
export async function checkCommand() {
    try {
        ...
        // 檢查無內容的筆記
        const emptyNotes = allNotes.filter(n => n.content.length === 0 || n.content.every(l => !l.content.trim()));
...
export async function statsCommand() {
    try {
        ...
        const totalBacklinks = allNotes.reduce((sum, n) => sum + (n.properties.backlink_count || 0), 0);
        output.subtitle('Links');
        output.item(`Total references: ${totalBacklinks}`);
        output.item(`Orphan notes: ${orphans.length}`);
        if (popular[0]) {
            output.item(`Most referenced: [[${popular[0].properties.id}]] (${popular[0].properties.backlink_count || 0})`);
        }
        output.blank();
        // 檔案統計
        const byFile = new Map();
